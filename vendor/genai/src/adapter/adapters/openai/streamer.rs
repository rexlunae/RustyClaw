...
impl OpenAIStreamer {
	...
	fn capture_tool_call(&mut self, index: usize, call_id: String, fn_name: String, arguments: String) -> ToolCall {
		...
		if let Some(existing_call) = calls.get_mut(index) {
			...
			// New tool call - fill any gaps with empty placeholders (NOT clones of
			// this call, which would poison earlier indices with the wrong id/name
			// and a partial argument fragment).
				...
				Some(Ok(Event::Message(message))) => {
					...
					if let Some(mut first_choice) = first_choice {
						...
						// -- Tool Call
							...
							&& delta_tool_calls != Value::Null
						{
							// Iterate ALL tool calls in the delta array. Some providers pack
								...
								for tool_call_obj_val in delta_tool_calls {
									...
									if let (Ok(index), Ok(mut function)) = (
										...
										if first_tool_call.is_none() {
											first_tool_call = Some(tool_call);
										}
									}
								}
							}
							if let Some(tool_call) = first_tool_call {
								// Return the ToolCallChunk event (all calls were captured above)

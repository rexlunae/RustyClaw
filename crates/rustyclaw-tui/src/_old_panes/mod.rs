...
// Re-export shared types from core so existing TUI code can still
// reference them as `crate::panes::GatewayStatus` etc.
pub use rustyclaw_core::types::{GatewayStatus, InputMode, MessageRole};
...
pub struct PaneState<'a> {
    pub config: &'a rustyclaw_core::config::Config,
    pub secrets_manager: &'a mut rustyclaw_core::secrets::SecretsManager,
    pub skill_manager: &'a mut rustyclaw_core::skills::SkillManager,
    pub soul_manager: &'a rustyclaw_core::soul::SoulManager,

...
fn setup_messaging(
    ...
    for def in MESSENGERS {
        println!("    {} — {}", t::accent_bright(def.display), t::muted(def.description));
    }
    println!();
            ...
            Some(pick) => {
                ...
                if def.id == "matrix" {
                    ...
                    let homeserver = prompt_line(
                        reader,
                        &format!("  {} ", t::accent("Homeserver URL (e.g., https://matrix.org):")),
                    )?;
                    let homeserver = homeserver.trim().to_string();
                    
                    if homeserver.is_empty() {
                        println!("  {}", t::icon_warn("No homeserver entered — skipping Matrix."));
                    ...
                    let token = prompt_secret(
                        reader,
                        &format!("  {} ", t::accent("Access token (or leave empty for password):")),
                    )?;
                    let token = token.trim().to_string();

                    let password = if token.is_empty() {
                        let pwd = prompt_secret(
                            reader,
                            &format!("  {} ", t::accent("Password:")),
                        )?;
                    ...
                    if token.is_empty() && password.as_ref().map(|p| p.is_empty()).unwrap_or(true) {
                        println!("  {}", t::icon_warn("No credentials entered — skipping Matrix."));
                    } else {
    ...
    if configured_messengers.is_empty() {
        ...
        println!("  {}", t::icon_ok(&format!(
            "Messengers configured: {}", names.join(", "),
        )));
    }
    ...
    println!();
    println!("  Want {} or other platforms?",
        t::accent_bright("Signal, iMessage, Slack, Instagram, LinkedIn"));
    println!();
    println!("  The {} skill connects to 15+ platforms",
        t::accent_bright("claw-me-maybe"));
    println!("  via {} — a free unified messaging app.",
    ...
    if selected == 1 {
        println!();
        println!("  {}", t::muted("You can install claw-me-maybe later with:"));
    ...
    if beeper_ready.trim().eq_ignore_ascii_case("s") {
        println!();
        println!("  {}", t::muted("Skipping. Install claw-me-maybe later when ready."));
        return Ok(configured_messengers);
    }
    ...
    let _ = prompt_line(
        reader,
        &format!("{} ", t::accent("Press Enter once enabled (or 's' to skip):")),
    )?;
    ...
    if skill_path.exists() {
        ...
        match output {
            ...
            Ok(out) => {
                ...
                println!("  Try manually: {}", t::accent_bright("clawhub install claw-me-maybe"));
            }
            Err(e) => {
...
fn setup_recommended_skills(
    ...
    if !clawhub_available {
        println!("  {}", t::warn("clawhub CLI not found."));
        println!("  Install it with: {}", t::accent_bright("npm install -g clawhub"));
        println!("  Then run {} to add skills.", t::accent_bright("rustyclaw skills install <name>"));
        println!();
        return Ok(());
    }
    ...
    if available.is_empty() {
        println!("  {}", t::icon_ok("All recommended skills are already installed."));
        println!();
        return Ok(());
    }

    // Group by category for display
    let mut by_category: std::collections::BTreeMap<&str, Vec<usize>> = std::collections::BTreeMap::new();
    for &i in &available {
        by_category.entry(RECOMMENDED_SKILLS[i].category).or_default().push(i);
    }
    ...
    for (category, indices) in &by_category {
        ...
        for &i in indices {
            ...
            println!("    {} — {}", t::accent_bright(skill.display), t::muted(skill.description));
        }
        println!();
    }

    // Let user select which to install
    let mut choices: Vec<String> = available.iter()
    ...
    let to_install: Vec<usize> = if selected == choices.len() - 1 {
        // Skip
        println!("  {}", t::muted("Skipping skill installation. You can install them later with:"));
        println!("    {}", t::accent_bright("rustyclaw skills install <name>"));
        println!();
        return Ok(());
    ...
    for &i in &to_install {
        ...
        let output = std::process::Command::new("clawhub")
            .args(["install", skill.slug, "--dir", &skills_dir.to_string_lossy()])
            .output();
        ...
        match output {
            ...
            Ok(out) => {
                let stderr = String::from_utf8_lossy(&out.stderr);
                println!("  {}", t::icon_warn(&format!(
                    "Failed to install {}: {}", skill.display, stderr.trim()
                )));
            }
            Err(e) => {
                println!("  {}", t::icon_warn(&format!(
                    "Failed to install {}: {}", skill.display, e
                )));
            }
        }
    }

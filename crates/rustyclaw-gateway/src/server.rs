...
//! back. Driven by [`run_gateway`] (networked) and [`run_stdio_gateway`]
//! (SSH-subsystem stdio mode). Invoked from the binary entry point in `main.rs`.
...
use rustyclaw_core::gateway::{
    ...
    GatewayOptions, ModelContext, ModelResponse, ProbeResult, ProviderRequest,
    ScopedTransportWriter, SecretEntryDto, ServerFrame, ServerFrameType, ServerPayload, StatusType,
    ToolCallResult, Transport, TransportAcceptor, WireFrame, deserialize_frame, protocol,
    transport,
};
...
use crate::{
    COMPACTION_THRESHOLD, SharedConfig, SharedCopilotSession, SharedModelCtx, SharedModelRegistry,
    SharedObserver, SharedSkillManager, SharedTaskManager, SharedVault, TOTP_LOCKOUT_SECS,
    ToolCancelFlag, auth, concurrent, errors, helpers, messenger_handler, providers, system_prompt,
    tool_executor,
};

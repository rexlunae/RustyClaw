...
// Re-export SSH server types
pub use ssh::{SshConfig, SshTransport, StdioTransport};

// Re-export model-connection probe
pub use providers::validate_model_connection;

// ── Gateway interface (wire protocol, transport, shared types) ──────────────
// These live in rustyclaw-core; the server crate consumes them. Bring the
// module aliases and the commonly-used types into scope so the server body can
// reference them unqualified, matching the pre-split layout.
use rustyclaw_core::gateway::{
    ChatMessage, ChatRequest, ClientFrame, ClientFrameType, ClientPayload, CopilotSession,
    GatewayOptions, MediaRef, ModelContext, ModelResponse, ProbeResult, ProviderRequest,
    ScopedTransportWriter, SecretEntryDto, ServerFrame, ServerFrameType, ServerPayload, StatusType,
    ToolCallResult, Transport, TransportAcceptor, WireFrame, deserialize_frame,
};
use rustyclaw_core::gateway::{protocol, transport};
...
// Protocol server-frame helpers (defined in core), used throughout the server.
use protocol::server::{
...
// ═══════════════════════════════════════════════════════════════════════════
//  Binary entry point (rustyclaw-gateway)
// ═══════════════════════════════════════════════════════════════════════════

// Binary-only imports (the server section above already brings Result, Config,
// GatewayOptions, ModelContext, SecretsManager, and CancellationToken into scope).

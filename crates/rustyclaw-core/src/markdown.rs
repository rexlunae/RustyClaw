//! Renderer-agnostic markdown to styled segment conversion.
//!
//! This module provides a lightweight markdown parser that produces
//! [`StyledSegment`] instances — a data structure that carries formatting
//! information (bold, italic, code, headers) without tying it to any
//! particular rendering framework.
//!
//! Both the TUI (iocraft) and desktop (Dioxus) clients can use the same
//! parser and then map `StyledSegment` to their own UI elements, or use
//! [`render_ansi`] for a simple ANSI-escape-code fallback.
//!
//! ## Supported syntax
//!
//! - **bold** and __bold__
...
//! - Fenced code blocks with language tags (triple backticks are matched
//!   as inline code blocks if opened and closed within the same segment)

/// A styled text segment produced by the markdown parser.
///
/// This is a renderer-agnostic intermediate representation. Each segment
/// carries its text content along with formatting flags. Consumers can
/// map these to framework-specific elements (e.g. Dioxus `span` with
/// classes, iocraft `Text` with styles, or ANSI escape sequences via
/// [`render_ansi`]).
#[derive(Debug, Clone)]
...
///
/// See module-level docs for supported syntax.
...
///
/// Useful for terminal clients that want a lightweight coloured
/// rendering without pulling in a full UI framework.

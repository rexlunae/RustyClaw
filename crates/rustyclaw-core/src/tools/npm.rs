...
pub async fn exec_npm_manage_async(args: &Value, workspace_dir: &Path) -> Result<String, String> {
    ...
    match action {
        "setup" | "install-node" => {
            if is_node_installed_async(workspace_dir).await && is_npm_installed_async(workspace_dir).await {
                let node_v = sh_in_async(workspace_dir, "node --version 2>&1").await.unwrap_or_else(|_| "unknown".into());
                let npm_v = sh_in_async(workspace_dir, "npm --version 2>&1").await.unwrap_or_else(|_| "unknown".into());
                return Ok(format!("Node.js ({}) and npm ({}) are already installed.", node_v.trim(), npm_v.trim()));
            }
        ...
        "version" | "versions" => {
            let node_v = sh_in_async(workspace_dir, "node --version 2>&1").await.unwrap_or_else(|_| "not installed".into());
            let npm_v = sh_in_async(workspace_dir, "npm --version 2>&1").await.unwrap_or_else(|_| "not installed".into());
            let npx_v = sh_in_async(workspace_dir, "npx --version 2>&1").await.unwrap_or_else(|_| "not installed".into());
            Ok(format!("node: {}\nnpm:  {}\nnpx:  {}", node_v.trim(), npm_v.trim(), npx_v.trim()))
        }
        ...
        "npm-install" | "add" | "i" => {
            ...
            if args.get("global").and_then(|v| v.as_bool()).unwrap_or(false) {
                cmd.push_str(" -g");
            }
            cmd.push_str(" 2>&1");
            sh_in_async(workspace_dir, &cmd).await
        ...
        "uninstall" | "remove" | "rm" => {
            ...
            if packages.is_empty() { return Err("No packages specified.".into()); }
            let global = if args.get("global").and_then(|v| v.as_bool()).unwrap_or(false) { " -g" } else { "" };
            sh_in_async(workspace_dir, &format!("npm uninstall {}{} 2>&1", packages.join(" "), global)).await
        }

        "list" | "ls" => {
            if !is_npm_installed_async(workspace_dir).await { return Err("npm is not installed.".into()); }
            let depth = args.get("depth").and_then(|v| v.as_u64()).unwrap_or(0);
            let global = if args.get("global").and_then(|v| v.as_bool()).unwrap_or(false) { " -g" } else { "" };
            sh_in_async(workspace_dir, &format!("npm list --depth={}{} 2>&1", depth, global)).await
        }
        ...
        "audit" => {
            ...
            let cmd = if fix { "npm audit fix 2>&1" } else { "npm audit 2>&1 || true" };
            sh_in_async(workspace_dir, cmd).await
        }
        ...
        "info" | "view" => {
            if !is_npm_installed_async(workspace_dir).await { return Err("npm is not installed.".into()); }
            let pkg = args.get("package").and_then(|v| v.as_str()).ok_or("Missing package")?;
        ...
        "status" => {
            ...
            let node_v = if node_installed {
                sh_in_async(workspace_dir, "node --version 2>&1").await.unwrap_or_else(|_| "unknown".into())
            } else { "not installed".into() };
            let npm_v = if npm_installed {
                sh_in_async(workspace_dir, "npm --version 2>&1").await.unwrap_or_else(|_| "unknown".into())
            } else { "not installed".into() };
            ...
            let has_node_modules = tokio::fs::try_exists(workspace_dir.join("node_modules")).await.unwrap_or(false);
            let has_lock = tokio::fs::try_exists(workspace_dir.join("package-lock.json")).await.unwrap_or(false)
                || tokio::fs::try_exists(workspace_dir.join("yarn.lock")).await.unwrap_or(false)
                || tokio::fs::try_exists(workspace_dir.join("pnpm-lock.yaml")).await.unwrap_or(false);
...
async fn sh_async(script: &str) -> Result<String, String> {
    ...
    if !output.status.success() && stdout.is_empty() {
        return Err(if stderr.is_empty() { format!("Command exited with {}", output.status) } else { stderr });
    }
    if !stderr.is_empty() && !stdout.is_empty() { Ok(format!("{}\n[stderr] {}", stdout, stderr)) }
...
async fn is_node_installed_async(workspace_dir: &Path) -> bool {
    sh_in_async(workspace_dir, "command -v node >/dev/null 2>&1 && echo yes").await.map(|s| s.contains("yes")).unwrap_or(false)
}

async fn is_npm_installed_async(workspace_dir: &Path) -> bool {
    sh_in_async(workspace_dir, "command -v npm >/dev/null 2>&1 && echo yes").await.map(|s| s.contains("yes")).unwrap_or(false)
}

// ── Sync implementations ────────────────────────────────────────────────────

fn sh(script: &str) -> Result<String, String> {
    let output = std::process::Command::new("sh").arg("-c").arg(script).output().map_err(|e| format!("shell error: {}", e))?;
    let stdout = String::from_utf8_lossy(&output.stdout).trim().to_string();
    let stderr = String::from_utf8_lossy(&output.stderr).trim().to_string();
    if !output.status.success() && stdout.is_empty() {
        return Err(if stderr.is_empty() { format!("Command exited with {}", output.status) } else { stderr });
...
fn sh_in(dir: &Path, script: &str) -> Result<String, String> {
    ...
    "#;
    let full = format!("{}\n{}", preamble.trim(), script);
    let output = std::process::Command::new("sh").arg("-c").arg(&full).current_dir(dir).output().map_err(|e| format!("shell error: {}", e))?;
    let stdout = String::from_utf8_lossy(&output.stdout).trim().to_string();
    let stderr = String::from_utf8_lossy(&output.stderr).trim().to_string();
    if !output.status.success() && stdout.is_empty() {
        return Err(if stderr.is_empty() { format!("Command exited with {}", output.status) } else { stderr });
...
fn is_node_installed(workspace_dir: &Path) -> bool {
    sh_in(workspace_dir, "command -v node >/dev/null 2>&1 && echo yes").map(|s| s.contains("yes")).unwrap_or(false)
}

fn is_npm_installed(workspace_dir: &Path) -> bool {
    sh_in(workspace_dir, "command -v npm >/dev/null 2>&1 && echo yes").map(|s| s.contains("yes")).unwrap_or(false)
}
    ...
    match action {
        "setup" | "install-node" => {
            if is_node_installed(workspace_dir) && is_npm_installed(workspace_dir) {
                let node_v = sh_in(workspace_dir, "node --version 2>&1").unwrap_or_else(|_| "unknown".into());
                let npm_v = sh_in(workspace_dir, "npm --version 2>&1").unwrap_or_else(|_| "unknown".into());
                return Ok(format!("Node.js ({}) and npm ({}) are already installed.", node_v.trim(), npm_v.trim()));
            }
            let os = std::env::consts::OS;
            match os {
                ...
                "linux" => sh("curl -fsSL https://raw.githubusercontent.com/nvm-sh/nvm/v0.40.1/install.sh | bash 2>&1 && \
                    export NVM_DIR=\"$HOME/.nvm\" && . \"$NVM_DIR/nvm.sh\" && nvm install --lts 2>&1"),
                _ => Err(format!("Unsupported OS: {}", os)),
            }
        }
        "version" | "versions" => {
            let node_v = sh_in(workspace_dir, "node --version 2>&1").unwrap_or_else(|_| "not installed".into());
            let npm_v = sh_in(workspace_dir, "npm --version 2>&1").unwrap_or_else(|_| "not installed".into());
            let npx_v = sh_in(workspace_dir, "npx --version 2>&1").unwrap_or_else(|_| "not installed".into());
            Ok(format!("node: {}\nnpm:  {}\nnpx:  {}", node_v.trim(), npm_v.trim(), npx_v.trim()))
        }
        "status" => {
            let node_installed = is_node_installed(workspace_dir);
            let npm_installed = is_npm_installed(workspace_dir);
            Ok(json!({ "node_installed": node_installed, "npm_installed": npm_installed }).to_string())
        }
        _ => Err(format!("Sync execution not fully supported for '{}'. Use async dispatch.", action)),
    }
}

...
pub async fn exec_system_monitor_async(
    ...
    if metric == "all" || metric == "cpu" {
        let load = sh_async("sysctl -n vm.loadavg 2>/dev/null || cat /proc/loadavg 2>/dev/null")
            .await
            .unwrap_or_default();
        result.insert("load_average".into(), json!(load.trim()));
        let top_cpu =
            sh_async("ps aux --sort=-%cpu 2>/dev/null | head -11 || ps aux -r | head -11")
                .await
                .unwrap_or_default();
    ...
    if metric == "all" || metric == "memory" {
        let mem = sh_async("vm_stat 2>/dev/null | head -10 || free -h 2>/dev/null")
            .await
            .unwrap_or_default();
        result.insert("memory".into(), json!(mem.trim()));
        let top_mem =
            sh_async("ps aux --sort=-%mem 2>/dev/null | head -11 || ps aux -m | head -11")
                .await
                .unwrap_or_default();
    ...
    if metric == "all" || metric == "network" {
        let net =
            sh_async("netstat -ib 2>/dev/null | head -5 || ip -s link 2>/dev/null | head -20")
                .await
                .unwrap_or_default();

//! Gateway tools: gateway, message, tts, image (sync entry points).
//!
...
pub(crate) fn merge_json(base: Value, patch: Value) -> Value {
    match (base, patch) {
...
pub fn exec_tts(args: &Value, workspace_dir: &Path) -> ToolResult {
    ...
    let mut file = fs::File::create(&output_path)
        .map_err(|e| ToolError::context("Failed to create output file", e))?;
...
pub fn exec_image(args: &Value, workspace_dir: &Path) -> ToolResult {
    ...
    let (image_data, media_type) = if is_url {
        ...
        let bytes =
            fs::read(&full_path).map_err(|e| ToolError::context("Failed to read image", e))?;

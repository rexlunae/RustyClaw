    ...
    match action {
        "interfaces" => {
            let output = if cfg!(target_os = "macos") {
                ...
                sh_async("ip -brief addr show 2>/dev/null || ifconfig 2>/dev/null | head -60").await?
            };
        ...
        "connections" => {
            ...
            let cmd = if cfg!(target_os = "macos") {
                if filter.is_empty() { "netstat -an | head -60".to_string() }
                else { format!("netstat -an | grep -i '{}' | head -60", filter) }
            } else if filter.is_empty() {
                "ss -tunapl 2>/dev/null | head -60 || netstat -tunapl 2>/dev/null | head -60".to_string()
            } else {
        ...
        "ping" => {
            ...
            Ok(json!({ "action": "ping", "target": host, "count": count, "output": output }).to_string())
        }
        ...
        "public_ip" => {
            let output = sh_async("curl -s --max-time 5 ifconfig.me 2>/dev/null || \
                curl -s --max-time 5 api.ipify.org 2>/dev/null").await?;
        ...
        "wifi" => {
            let output = if cfg!(target_os = "macos") {
                ...
                sh_async("iwconfig 2>/dev/null || nmcli device wifi list 2>/dev/null | head -30").await?
            };
...
pub async fn exec_net_scan_async(args: &Value, _workspace_dir: &Path) -> Result<String, String> {
    ...
    match action {
        "nmap" => {
            ...
            let scan_type = args.get("scan_type").and_then(|v| v.as_str()).unwrap_or("quick");
            ...
            let cmd = match scan_type {
                "quick" => {
                    if let Some(p) = ports { format!("nmap -T4 -p {} {} 2>&1", p, host) }
                    else { format!("nmap -T4 -F {} 2>&1", host) }
                }
                "full" => format!("nmap -T4 -p- {} 2>&1", host),
                "service" | "version" => {
                    if let Some(p) = ports { format!("nmap -sV -p {} {} 2>&1", p, host) }
                    else { format!("nmap -sV -F {} 2>&1", host) }
                }
                "os" => format!("sudo nmap -O {} 2>&1", host),
                "udp" => {
                    if let Some(p) = ports { format!("sudo nmap -sU -p {} {} 2>&1", p, host) }
                    else { format!("sudo nmap -sU --top-ports 20 {} 2>&1", host) }
                }
        ...
        "tcpdump" => {
            let iface = args.get("interface").and_then(|v| v.as_str()).unwrap_or("any");
            let filter = target.unwrap_or("");
            ...
            let cmd = if filter.is_empty() {
                ...
                format!("sudo timeout 10 tcpdump -i {} -c {} -nn '{}' 2>&1", iface, count, filter)
            };
            let output = sh_async(&cmd).await?;
        ...
        "port_check" => {
            ...
            if let Some(p) = port {
                let cmd = format!("nc -z -w 3 {} {} 2>&1 && echo 'OPEN' || echo 'CLOSED'", host, p);
                let output = sh_async(&cmd).await?;
                Ok(json!({
        ...
        "listen" => {
            let output = if cfg!(target_os = "macos") {
                ...
                sh_async("ss -tlnp 2>/dev/null | head -40 || netstat -tlnp 2>/dev/null | head -40").await?
            };
        ...
        "sniff" => {
            ...
            let cmd = match tool.as_deref() {
                Some("tcpdump") => format!("sudo timeout {} tcpdump -i {} -c 30 -q -nn 2>&1", seconds, iface),
...
pub fn exec_net_scan(args: &Value, _workspace_dir: &Path) -> Result<String, String> {
    ...
    match action {
        ...
        "port_check" => {
            ...
            let cmd = format!("nc -z -w 3 {} {} 2>&1 && echo 'OPEN' || echo 'CLOSED'", host, port);
            let output = sh(&cmd)?;

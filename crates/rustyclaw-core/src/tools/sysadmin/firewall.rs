    ...
    match action {
        "status" => {
            let cmd = match backend {
                ...
                _ => return Err("No supported firewall found".to_string().into()),
            };
        ...
        "allow" => {
            ...
            let cmd = match backend {
                ...
                "firewalld" => format!(
                    "sudo firewall-cmd --add-port={}/{} --permanent 2>&1 && sudo firewall-cmd --reload 2>&1",
                    port, proto
                ),
                "iptables" => format!(
        ...
        "deny" => {
            ...
            let cmd = match backend {
                ...
                "firewalld" => format!(
                    "sudo firewall-cmd --remove-port={}/{} --permanent 2>&1 && sudo firewall-cmd --reload 2>&1",
                    port, proto
                ),
                "iptables" => format!(
